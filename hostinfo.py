...
class SystemInfo(object):
	...
	def __init__(self):
		...
		self.database = Database(dbhost, dbuser, dbpass, dbname)
	...
	def sethostinfo(self, warranty):
		...
		if len(self.database.curs.fetchall()) < 1:
			ins_sqlcode='''INSERT INTO warranty (hostname, SKU, system_type,
				serialnum, vendor) VALUES(%s,%s,%s,%s,%s)'''

			try:
				self.database.curs.execute(ins_sqlcode, (warranty["hostname"],
					warranty["SKU"], warranty["type"], warranty["serial"],
					warranty["vendor"]))
			except Exception, e:
...
if vendor == "ibm":
	type = server.product_name.split("[")[1][0:4]
else:
	type = server.product_name

serial = server.serial
SKU = server.SKU

warranty_info = {"SKU" : "N/A", "type" : type, "serial" : serial,
	"hostname" : getfqdn(), "vendor" : vendor}

if vendor == "hp":
	warranty_info["SKU"] = SKU
